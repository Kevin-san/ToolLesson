...
def pages_help_by_zero(page,num_pages,category_id,maxpage):
    ...
    if num_pages > maxpage and offset <= maxpage and p>= maxpage:
        ...
        return get_pages_by_zero(num_pages - maxpage, num_pages,category_id,num_pages)
    elif num_pages <= maxpage:
        #假设3页  3<6，总页数很少，少于规定页数
        # 当前页码数小于规定数
        # print("当前页码数小于规定数",[i + 1 for i in range(num_pages)])
        return get_pages_by_zero(0, num_pages,category_id,num_pages)
    else:
        # 正常页数分配
        # print("正常页数分配",[i + 1 for i in range(p - int(maxpage / 2), p + int(maxpage / 2))])
        return get_pages_by_zero(p-1, p + maxpage-1 ,category_id,num_pages)
...
def get_media_content(media_type,media_id,order_no):
    ...
    pages = pages_help_by_zero(int(order_no)+1,total_count,F"content/{media_id}",10)
