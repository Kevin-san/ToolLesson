...
from PdfWeb.models import BookLessonType,BookLesson,Chapter,Content,ImageContent,\
    CommonRules
from PdfWeb.entitys import HomeInfoItem

def get_book_lesson_type_by_id(book_lesson_type_id):
    return BookLessonType().objects.get(id=book_lesson_type_id)

def get_book_lesson(book_lesson_id):
    return BookLesson().objects.get(id=book_lesson_id)
...
def get_book_lesson_info(book_lesson_type_id):
    return BookLesson.objects.filter(LessonTypeId=book_lesson_type_id,DeleteFlag=0)

def get_chapter_infos(book_lesson_id):
    return Chapter.objects.filter(BookLessonId=book_lesson_id,DeleteFlag=0).order_by('ChapterNo')
...
def get_content_infos(chapter_id):
    return Content.objects.filter(ChapterId=chapter_id,DeleteFlag=0).order_by('OrderIndex')
...
def get_common_rules():
    return CommonRules.objects.filter(DeleteFlag=0).order_by('Id')

def get_common_rules_by_type(rule_type):
    return CommonRules.objects.filter(TypeKey=rule_type,DeleteFlag=0).order_by('Id')

def get_common_rules_by_type_and_rule(rule_type,rules):
    return CommonRules.objects.filter(TypeKey=rule_type,Rules=rules,DeleteFlag=0).order_by('Id')
