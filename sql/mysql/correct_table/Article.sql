...
create table IF NOT EXISTS `Article`( -- 博客文章表
	...
	PRIMARY KEY ( `Id` ),
	CONSTRAINT `FK_article_user` FOREIGN KEY (`AuthorId`) REFERENCES `user` (`Id`) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT `FK_article_category` FOREIGN KEY (`CategoryId`) REFERENCES `category` (`CategoryId`) ON UPDATE CASCADE ON DELETE CASCADE,
