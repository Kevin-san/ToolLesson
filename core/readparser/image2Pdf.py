...
def convert_images2pdf_one_dir(file_dir,save_name=None ,filename_sort_fn=None):
    ...
    for parent, dirnames ,filenames in os.walk(file_dir):
        ...
        if len(book_pages) > 0 :
            ...
            end_time = time.clock()
            print("[*][转换PDF] : 结束. [保存路径] > [%s] , 耗时 %f s " % (save_name, (end_time - begin_time)))
        else :
            print("该目录下没有找到任何图片文件.如果是多重目录,尝试使用 convert_images2PDF_more_dirs 函数")


def convert_images2pdf_more_dirs(dir_path):
    """
    转换一个目录文件夹下的图片至 PDF
    :param file_dir:
    :param filename_sort_fn:
    ...
    for parent, dirnames, filenames in os.walk(dir_path):
        for dirname in dirnames:
            # 假设每个文件夹下都有图片，都是一本书
            dir_data = {"name": "", "pages": [], "isBook": False}
            dir_name = dirname.split('/')[0]
            dir_data['name'] = dir_name
            __dirs[dir_name] = dir_data
        # 查找有无图片
        for filename in filenames:
            real_filename = os.path.join(parent, filename)
            # 取父文件夹名称为书名
            parent_dir_name = real_filename.split('/')[-2]
            if parent_dir_name in __dirs.keys():
                dir_json_data = __dirs[parent_dir_name]
            else:
                continue
            # 检查是否图片
            if __is_allow_file(real_filename) :
                # 将图片添加至书本
                dir_json_data['pages'].append(real_filename)
                # 如果该书的isbook 是false 改为true
                if not dir_json_data['isBook']:
                    dir_json_data['isBook'] = True
    index = 1
    for dir_name in __dirs.keys():
        dir_data = __dirs[dir_name]
        if dir_data['isBook']:
            print("[*][转换PDF] : 开始. [名称] > [%s]" % (dir_name))
            begin_time = time.clock()
            __converted(os.path.join(dir_path,(dir_data['name'] + ".pdf")) , dir_data['pages'])
            end_time = time.clock()
            print("[*][转换PDF] : 结束. [名称] > [%s] , 耗时 %f s " % (dir_name, (end_time - begin_time)))
            index += 1
    print("[*][所有转换完成] : 本次转换检索目录数 %d 个，共转换的PDF %d 本 " % (len(__dirs), index - 1))


def __is_allow_file(filepath):
    """
    是否允许的文件
    :param file:
    :return:
...
def __converted(save_book_name,book_pages=[],filename_sort_fn=None):
    ...
    book_pages_data = []
    book_doc = SimpleDocTemplate(save_book_name, pagesize=A4, rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=18)
    for page in book_pages:
        ...
        book_pages_data.append(data)
        book_pages_data.append(PageBreak())
    try:
        book_doc.build(book_pages_data)
        # print("已转换 >>>> " + bookName)
