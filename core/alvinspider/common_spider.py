...
def get_response(artifact_url,user,password,referer_url='',proxies=''):
    ...
    if user == '' and password == '':
        response = requests.get(url=artifact_url,proxies=proxies,headers=headers,timeout=(30,35),verify=False,stream=True)
    else:
        response = requests.get(url=artifact_url,proxies=proxies,headers=headers,timeout=(30,35),verify=False,auth=(user,password),stream=True)
